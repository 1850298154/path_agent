...
class ValidityChecker(ob.StateValidityChecker): # 作为函数，提供给 og：  solved = optimizingPlanner.solve(runtime)  #ompl-geometric-SimpleSetup.cpp 第116行;返回值为lastStatus_ # zyt:path
    ...
    def isValid(self, state):
        ...
        if SET.path_obstacle_list[x][y]==0:# zyt:path
            return True 
        else:
            return False
...
def path_plan(ini,target):
    ...
    optimizingPlanner = og.ABITstar(si) # zyt:path
    ...
    for i in range(5):
        ...
        solved = optimizingPlanner.solve(runtime)  #ompl-geometric-SimpleSetup.cpp 第116行;返回值为lastStatus_ # zyt:path
...
if __name__ == '__main__':
    ...
    SET.initialize_set()  # 初始化设置文件SET

    SET.extend_path_plan_obstacle_list=Build_ExtensionZone(SET.path_obstacle_list,0.1)

    # start=time.time()

    # for i in range(10000):
    #     C=np.random.rand(2)*np.array([5.0,4.0])
    #     detect_point_in(SET.path_plan_obstacle_list,np.array([4.3,2.4]))
    ...
    target=np.array([4.8,3.8])

    
    path = path_plan(start,target,SET.extend_path_plan_obstacle_list)
    print(path)

    plt.figure(figsize=(10,8))
    plot_obstacle(SET.extend_path_plan_obstacle_list,extend=True)
