...
if not agents_starts:
    print('Warning: No agent start points loaded, generating defaults')
    agents_starts = []
    for i in range(Num):
        x = random.uniform(radius, set_xlim - radius)
        y = random.uniform(radius, set_ylim - radius)
        agents_starts.append((x, y))
if not agents_ends:
    print('Warning: No agent end points loaded, generating defaults')
    agents_ends = []
    for i in range(Num):
        x = random.uniform(radius, set_xlim - radius)
        y = random.uniform(radius, set_ylim - radius)
        agents_ends.append((x, y))
