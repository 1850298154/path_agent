...
def sort_obstacles_by_minimum_distance(obstacle_list, agent):
    sorted_obstacles = sorted(obstacle_list, key=lambda obstacle: obstacle.get_minimum_distance(agent.p))
...
def Get_ob_cons(agent, obstacle_list):
    ...
    # new_obstacle_list = []
    # for i in range(len(obstacle_list)):
    #     if obstacle_list[i].get_minimum_distance(agent.p) > agent.r_max:
    #         # del obstacle_list[j]
    #         pass
    #     else:
    #         new_obstacle_list.append(obstacle_list[i])
    #         # j = j+1
    # obstacle_list = new_obstacle_list

    # obstacle_list 为障碍物列表
    sorted_obstacles = sort_obstacles_by_minimum_distance(obstacle_list, agent)
    # sorted_distances = [obstacle.get_minimum_distance(agent.p) for obstacle in sorted_obstacles]
    i=0  #如果没有障碍物 ， for 循环就不会产生i变量
    for i in range(len(sorted_obstacles)):
        if sorted_obstacles[i].get_minimum_distance(agent.p) > outer_r_multi* agent.r_max:
            break
    # print('sorted_distances')
    # print(sorted_distances)
    obstacle_list=sorted_obstacles[:min(max(i,1),2)]

    # print('agent.index, len(new_obstacle_list)')
    # print(agent.index, len(obstacle_list))
    ...
    segment_list = get_segment_list(
        obstacle_list, pre_traj_augment)  # 对预测轨迹判断是否与障碍物相交，会抛出自定义异常
    ...
    # 将每一个走廊里的点都包装为一个多边形块
    segment_list_polygon = get_segment_list_polygon(
        segment_list, pre_traj_augment)
    start = Time.time()
    # 将多边形块与障碍物之间通过线性约束生成走廊
    corridor = []
    for seg in segment_list_polygon:
        corridor += [get_polyhedron(obstacle_list, seg)]  # cvxopt求解SVM
...
def get_polyhedron(obstacle_list, segment_polygon):
    ...
    for i in range(len(obstacle_list)):
        ...
        if flag:
            ...
            #     # print(f'    agent   {agent.index}   meet deadlock    ')
            #     print("An exception has occurred  :  ", str(e))
            #     print('**********************************************')
            #     # agent.deadlock = True
            #     # agent.cache = SVM_fall_back(agent)
            plane = SVM(obstacle.vertex_list, segment_polygon.vertex_list)
...
def get_segment_list(obstacle_list, pre_traj_augment):
    ...
    for _ in range(len(pre_traj_augment)+1):  # 分段数量不应该超过总的视界长度
        ...
        for point in range(segment_begin-1, -1, -1):
            ...
            # 1层 segment_begin
            # 2层 point
            # vertex_list = pre_traj_augment[segment_begin: (point-1): -1]
    ...
    print(segment_list)
    l = line(pre_traj_augment[segment_list[-1][0]],
             pre_traj_augment[segment_list[-1][0]-1])
...
if __name__ == '__main__':
    ...
    polygon2 = [
        [42.346882740439774, 81.98015853755194],
        [42.346882740439774, 81.98015853755194],
        [42.34062067195161, 81.93589513570913],
        [42.32754400350994, 81.8434627371803],
        [42.307652667297454, 81.70286135156195],
        [42.28094671632346, 81.5140909713579],
        [42.24742612115648, 81.27715160073727],
        [42.20709090073079, 80.99204323703201],
    ]
    get_segment_list(polygon1, polygon2)
